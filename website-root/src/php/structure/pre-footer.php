<section class="tfac-pre-footer">
  <div class="mdev-main-wrapper">
      <h5 class="u-uppercase">More to do at <span class="tfac-green u-bold">The Factory</span></h5>
      <!-- Image Strip -->
      <?php include 'modules/image-strip-sm.php'; ?>

      <div class="flex flex-row flex-hor-between flex-no-wrap">

        <div class="tfac-pre-column">
          <h2 class="u-uppercase"><span class="tfac-green u-bold">Canada's Biggest</span> Indoor adventure park</h2>
        </div>

        <div class="tfac-pre-column">
          <h5 class="u-uppercase u-bold">I'm Ready to Book!</h5>
          <p>Already know what you want to do? Click "Book Online" to guarantee your spot and get the time that's best for you.  If you need to book a party room for your event, click "Book Event" to get started.</p>
          <div class="tfac-column-action flex flex-start flex-hor-between flex-vert-center">
            <a href="#" class="tfac-btn tfac-main-cta">Book Online<i class="tfac-btn-arrow"></i></a>
            <a href="#" class="tfac-btn tfac-main-cta">Book Event<i class="tfac-btn-arrow"></i></a>
          </div>
        </div>

      <div class="tfac-pre-column">
        <!-- Help Links -->
        <?php include 'modules/help-links.php' ?>
      </div>

    </div>
  </div>
</section>

